        ...
        match msg {
            Message::PaneFocusCycle(window_id, grid_id, pane) => {
                ...
                if let Some(state) = self.states.get(&key) {
                    ...
                    HandleResult::emit(vec![OutgoingEvent::pane_focus_cycle(
                        grid_id.clone(),
                        pane_id,
                    )])
                } else {
            ...
            Message::PaneResized(window_id, grid_id, evt) => {
                ...
                HandleResult::emit(vec![OutgoingEvent::pane_resized(
                    grid_id.clone(),
                    format!("{:?}", evt.split),
                    evt.ratio,
                )])
            ...
            Message::PaneDragged(window_id, grid_id, evt) => {
                ...
                match evt {
                    pane_grid::DragEvent::Picked { pane } => {
                        if let Some(state) = self.states.get(&key) {
                            let pane_id = state.get(*pane).cloned().unwrap_or_default();
                            HandleResult::emit(vec![OutgoingEvent::pane_dragged(
                                grid_id.clone(),
                                "picked",
                                pane_id,
                                None,
...
fn find_pane_by_id(state: &pane_grid::State<String>, pane_id: &str) -> Option<pane_grid::Pane> {
    state
        .panes
        .iter()
...
fn render_pane_grid_with_state<'a, R: PlushieRenderer>(
    ...
    for c in &node.children {
        ...
        if let Some(title) = prop_str(&c.props, "title") {
