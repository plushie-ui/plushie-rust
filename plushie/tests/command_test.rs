...
fn command_focus_carries_id() {
    match Command::focus("email") {
        Command::Focus(id) => assert_eq!(id, "email"),
...
fn command_close_window_produces_window_op() {
    match Command::close_window("main") {
        Command::Window(WindowOp::Close(id)) => assert_eq!(id, "main"),
...
fn command_clipboard_read() {
    match Command::clipboard_read("paste") {
        Command::Effect { tag, request: EffectRequest::ClipboardRead } => {
            assert_eq!(tag, "paste");
        }
        _ => panic!("expected Effect(ClipboardRead)"),
    }
}
...
fn command_widget_command_carries_payload() {
    ...
    match cmd {
        ...
        _ => panic!("expected WidgetCommand"),
    }
}
...
fn commands_are_inspectable_for_testing() {
    ...
    assert!(matches!(cmd, Command::Focus(ref id) if id == "email"));
}
