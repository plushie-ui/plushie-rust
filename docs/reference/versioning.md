...
same string (for example `plushie-core = { version = "0.7.1", path
...
requirement would allow `^0.7.1` to roll forward to `0.8.0`, which
...
`PLUSHIE_RUST_VERSION=0.7.1` is compatible with any Rust app that
pins `plushie = "=0.7.1"` in its `Cargo.toml`.

## Upgrade guidance

- Patch upgrades (`0.7.1 -> 0.7.2`): bump the pin, rebuild,
