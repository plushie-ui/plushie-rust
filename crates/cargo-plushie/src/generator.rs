...
fn render_main_rs(config: &WorkspaceConfig<'_>) -> String {
    ...
    for w in config.widgets {
        body.push_str(&format!(
            "\n    registry.register_strict(Box::new({}));",
            w.constructor
        ));
    }
    body.push_str("\n    registry\n}\n\n");
...
mod tests {
    ...
    fn renders_main_rs_registers_every_widget() {
        ...
        assert!(main_rs.contains("fn registry() -> WidgetRegistry"));
        assert!(main_rs.contains("registry.register_set"));
        assert!(main_rs.contains("registry.register_strict(Box::new(my_gauge::Gauge::new()))"));
        assert!(main_rs.contains(".with_session_factory(registry)"));
