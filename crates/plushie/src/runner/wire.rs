...
fn wire_to_sdk_event(msg: &Value, effect_tracker: &mut EffectTracker) -> Option<Event> {
    ...
    let sink_event = match msg_type {
        ...
        "interact_response" => {
            // Interact responses contain events generated by the
            // renderer's interaction handler. Each event is
            // processed as a normal renderer event.
            let events = msg.get("events").and_then(|v| v.as_array());
            if let Some(events) = events {
                // Process all but the last event and return the last
                // one to the caller for normal MVU processing.
                // If there are no events, return None.
                if events.is_empty() {
                    return None;
                }
                // For simplicity, convert the first event and let
                // the caller process it. Multi-event interact
                // responses will need a more sophisticated approach.
                return wire_to_sdk_event(&events[0], effect_tracker);
            }
            return None;
        }
