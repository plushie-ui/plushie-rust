...
impl<R: PlushieRenderer> PlushieWidget<R> for ColumnWidget {
    ...
    fn render<'a>(
        ...
        let width = cp.width.as_ref().map(iced_convert::length).unwrap_or(iced::Length::Shrink);
        let height = cp.height.as_ref().map(iced_convert::length).unwrap_or(iced::Length::Shrink);
        let align_x = cp
            .align_x
