...
impl App for Gallery {
    ...
    fn view(model: &Self, _widgets: &mut WidgetRegistrar) -> View {
        ...
        window("main").title("Widget Gallery").child(
            scrollable().id("gallery").height(Fill).child(
                column().padding(16).spacing(16.0)
                    ...
                    .child(text_input("input", &model.input_value)
                        .placeholder("Type here..."))
                    .child(text_input("submit-input", &model.submit_value)
                    ...
                    .child(text(&format!("Value: {}", model.slider_value as i32))
                        .id("slide-value").size(12.0))
                    // Selection
                    .child(section("Selection"))
                    .child(pick_list("pick",
                        &["Apple", "Banana", "Cherry"],
                        Some(&model.selected)))
                    ...
                    .child(text("Styled text")
                        .id("styled").size(18.0).color(Color::hex("#3b82f6"))),
            ),
        ).into()
    }
}
